...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space):
        ...
        self.factor = None
        # Optional per-step credit weights of shape [T, N, 1]
        self.credit_weights = None
    ...
    def update_credit(self, credit_weights):
        assert credit_weights.shape[0] == self.episode_length
        assert credit_weights.shape[1] == self.n_rollout_threads
        assert credit_weights.shape[-1] == 1
        self.credit_weights = credit_weights.astype(np.float32, copy=True)
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        if self.credit_weights is not None:
            cw = self.credit_weights.reshape(-1, 1)
            advantages = advantages * cw
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, n_rollout_threads, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                if self.credit_weights is not None:
                    credit_batch.append(self.credit_weights[:, ind])
            ...
            if self.credit_weights is not None and len(credit_batch) > 0:
                credit_batch = np.stack(credit_batch, 1)
                adv_targ = adv_targ * credit_batch
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        # Apply credit weights if provided
        if self.credit_weights is not None:
            credit = _cast(self.credit_weights)
