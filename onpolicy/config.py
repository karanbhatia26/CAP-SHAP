...
def get_config():
    ...
    # Phase segmentation + credit assignment (research prototype)
    parser.add_argument("--use_phase_credit", action='store_true', default=False,
                        help="Enable phase-wise Shapley-style credit weights applied to advantages")
    parser.add_argument("--phase_method", type=str, default='reward_changepoint', choices=['reward_changepoint', 'fixed'],
                        help="Rule for phase splitting")
    parser.add_argument("--phase_threshold", type=float, default=0.5,
                        help="Threshold factor for reward change-point detection")
    parser.add_argument("--phase_min_len", type=int, default=10,
                        help="Minimum length of a phase")
    parser.add_argument("--phase_fixed_num", type=int, default=3,
                        help="If method=fixed, number of phases")
    parser.add_argument("--phase_smooth_window", type=int, default=5,
                        help="Smoothing window for reward change-point detection")
    # Credit assignment method
    parser.add_argument("--credit_method", type=str, default='proxy', choices=['proxy', 'deep'],
                        help="Method for computing Shapley credit: proxy (action-based) or deep (neural network)")
    parser.add_argument("--credit_decay_lambda", type=float, default=0.5,
                        help="Temporal aggregator decay; later phases get larger weight when < 1")
    parser.add_argument("--noop_action", type=int, default=0,
                        help="Index to treat as NOOP when using proxy Shapley credit with discrete actions")
    # Deep Shapley parameters
    parser.add_argument("--deep_shapley_lr", type=float, default=1e-3,
                        help="Learning rate for deep Shapley network")
    parser.add_argument("--deep_shapley_hidden", type=int, default=64,
                        help="Hidden dimension for deep Shapley network")
    parser.add_argument("--deep_shapley_buffer", type=int, default=1000,
                        help="Buffer size for deep Shapley training")
    parser.add_argument("--deep_shapley_coalitions", type=int, default=8,
                        help="Number of coalitions to sample per training step")
    # Logging for phase credit visualization
    parser.add_argument("--log_phase_credit", action='store_true', default=False,
                        help="Log phase credit stats and per-agent averages")
    parser.add_argument("--log_phase_hist", action='store_true', default=False,
                        help="Also log histogram of credit weights")
